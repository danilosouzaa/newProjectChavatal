...
Instance *allocationStructInstance(int number_Variables, int number_Constraints)
{
    ...
    inst->ub_variables = (int*)(inst->name_constraints + number_Constraints);
    inst->lb_variables = (int*)(inst->ub_variables+ number_Variables);
...
Instance *readLP(char *fileName)
{
    ...
    {   int test = 0;
        while(!feof(arq))
        {
            ...
            if((atv==1)&&(strcmp(linha,"Generals")!=0))
            {
                strcpy(inst->name_variables[cont].name,linha);

                if(test == 0){
                    inst->type_variables[cont] = 1;
                }else{
                    inst->type_variables[cont] = 0;
                }
                cont++;
            }
            p = strchr(linha,':');
            if(p!=NULL)
            {
                strncpy(inst->name_constraints[cont2].name, linha, strlen(linha)-1);
                cont2++;
            }
            if(strcmp(linha,"Binaries")==0)
            {
